...
module Day12 = 
    ...
    let calculateVelocity (planet: Planet) (planets: Planet list) =               
        planets |> List.fold (fun agg curr -> {agg with X=agg.X + calculateNewVelocity curr.Position.X planet.Position.X; Y=agg.Y + calculateNewVelocity curr.Position.Y planet.Position.Y; Z=agg.Z + calculateNewVelocity curr.Position.Z planet.Position.Z}) {X=0; Y=0; Z=0}
    ...
    let part1 planets runCount =
        runFor planets runCount |> List.map (fun x -> (Math.Abs(x.Position.X) + Math.Abs(x.Position.Y) + Math.Abs(x.Position.Z)) * (Math.Abs(x.Velocity.vX) + Math.Abs(x.Velocity.vY) + Math.Abs(x.Velocity.vZ))) |> List.sum